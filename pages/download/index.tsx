...
const DownloadPage = () => {
    ...
    useEffect(() => {
        const fetchBuilds = async () => {
            try {
                const response = await axios.get('https://parser.vm-sandro-cdn.secureedumail.xyz:8443/get-releases');
        ...
        <div className="container mx-auto p-4">
            ...
            <nav className="flex items-center flex-col flex-wrap md:flex-row justify-between pt-4"
                ...
                <span
                    ...
                    Montre <span
                    className="font-semibold text-gray-900 dark:text-white">{(currentPage - 1) * itemsPerPage + 1}</span> vers <span
                    className="font-semibold text-gray-900 dark:text-white">{Math.min(currentPage * itemsPerPage, filteredBuilds.length)}</span> de <span
